...
abstract Promise<T>(Surprise<T, Error>) from Surprise<T, Error> to Surprise<T, Error> {
  ...
  static public final NOISE:Promise<Nada> = Future.sync(Success(Nada));
  @:deprecated('use Promise.NOISE instead') static public final NULL:Promise<Nada> = NOISE;
  ...
  @:to public function noise():Promise<Nada>
    return 
      if (this.status.match(NeverEver)) cast NEVER;
      else (this:Promise<T>).next(function (v) return Nada);
   ...
   *
   *   If this function's returned promised resolves to an `Error`, this retry will abort with such error. Otherwise if it resolves to a `Success(Nada)`, the retry will continue.
   *
   *   Some usage examples:
   ...
   *         case 10: info.error;
   *         case v: Future.delay(v * 1000, Nada);
   *     }
   *     ```
   *
   ...
   *       default: Future.delay(1000, Nada);
   *     }
   *     ```
   *
   ...
   *       else Future.delay(1000, Nada);
   *     ```
   *
   * @return Promise<T>
   */
  @:noUsing
  static public function retry<T>(gen:()->Promise<T>, next:Next<{ attempt: Int, error:Error, elapsed:Float }, Nada>):Promise<T> {
  ...
  #if (!java || jvm)
  @:noUsing
  static public function cache<T>(gen:()->Promise<Pair<T, Future<Nada>>>):()->Promise<T> {
    var p = null;
    return function() {
      var ret = p;
      if(ret == null) {
