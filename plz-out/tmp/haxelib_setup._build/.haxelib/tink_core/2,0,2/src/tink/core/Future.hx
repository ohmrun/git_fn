...
abstract Future<T>(FutureObject<T>) from FutureObject<T> to FutureObject<T> from FutureTrigger<T> {
  ...
  static public final NOISE:Future<Nada> = Future.sync(Nada);
  @:deprecated('use Future.NOISE instead') static public final NULL:Future<Nada> = NOISE;
  ...
  **/
  ...
  @:to public function noise():Future<Nada>
    return
